...
R_InitSpriteDefs(char **namelist)
{
  ...
  while (*check != NULL)
    ...
    // FIXME: HACK: this causes overflow, reduce numsprites until it works..
#ifdef armv7hf
  numsprites = check - namelist - 21;
#else
  numsprites = check - namelist - 11;
#endif
...
#ifdef RANGECHECK
  if ((unsigned)thing->sprite >= (spritenum_t)numsprites)
...
#ifdef RANGECHECK
  if ((unsigned)psp->state->sprite >= (spritenum_t)numsprites)
