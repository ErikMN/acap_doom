...
M_DrawOptions(void)
{
  ...
  V_DrawPatchDirect(
      OptionsDef.x + 175, OptionsDef.y + LINEHEIGHT * detail, 0, W_CacheLumpName(detailNames[detailLevel], PU_CACHE));

  V_DrawPatchDirect(
      OptionsDef.x + 120, OptionsDef.y + LINEHEIGHT * messages, 0, W_CacheLumpName(msgNames[showMessages], PU_CACHE));
...
M_Drawer(void)
{
  ...
  // DRAW SKULL
  V_DrawPatchDirect(
      x + SKULLXOFF, currentMenu->y - 5 + itemOn * LINEHEIGHT, 0, W_CacheLumpName(skullName[whichSkull], PU_CACHE));
}
...
M_Init(void)
{
  ...
  case commercial:
    ...
    MainMenu[readthis] = MainMenu[readthis]; // quitdoom
    MainDef.numitems--;
    MainDef.y += 8;
